...
def presence_registre(request):
    ...
    # Filtre service
    from employer.models import Departement
    services   = Departement.objects.order_by('nom')
    service_id = request.GET.get('service', '')
    service_sel = None
    employes = _employes_actifs()
    if service_id:
        try:
            service_sel = Departement.objects.get(pk=service_id)
            employes = employes.filter(services=service_sel)
...
def presence_employe(request, pk):
    ...
    employe = get_object_or_404(Employe.objects.select_related('fonction').prefetch_related('services'), pk=pk)
    today = date.today()
    try:
        year  = int(request.GET.get('year',  today.year))
        month = int(request.GET.get('month', today.month))
...
def presence_stats(request):
    ...
    # Par service
    stats_svc = []
    for svc in Departement.objects.order_by('nom'):
        emps = _employes_actifs().filter(services=svc)
        n = emps.count()
        if not n:
            continue
...
def presence_chercher(request):
    ...
    try:
        emp = Employe.objects.select_related('fonction').prefetch_related('services').get(
    ...
    return JsonResponse({
        ...
        'emp': {
            ...
            'service':    ', '.join(s.nom for s in emp.services.all()) or '—',
