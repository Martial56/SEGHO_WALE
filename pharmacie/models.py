...
# ── Ce qui vivait ici ─────────────────────────────────────────────────────
# `CategorieMedicament`, `Medicament`, `LotMedicament`, `CommandePharmacies` et
# un `MouvementStock` homonyme de celui de l'app `stock` formaient la chaîne
# d'origine (9 mai 2026),
# écrite quand le déploiement n'avait qu'une pharmacie : un seul `stock_actuel`
# par médicament, sans notion de centre. La réécriture du 30 mai lui a substitué
# `stock.Produit` (le catalogue et la réserve centrale) et `StockPharmacie` (une
# quantité par pharmacie), mais l'ancienne est restée branchée quatre mois sur
# les ordonnances, avec des prix qui avaient divergé — 50 F le paracétamol côté
# prescription, 25 F côté comptoir. Ses tables sont vides et ses lecteurs sont
# passés à la nouvelle : elle est supprimée par les migrations 0011 à 0013.
#
# `CommandePharmacies` portait 24 en-têtes de commande fournisseur sans aucune
# ligne de détail, qu'aucun écran n'ouvrait — le module `achats` fait ce travail
# depuis. Ses 24 lignes ont été exportées avant suppression.
...
class MouvementPharmacie(models.Model):
    ...
    TYPE_CHOICES = [
        ...
        ('vente',       'Vente caisse'),
        # La facturation est une seconde porte de sortie, distincte du comptoir :
        # les deux se lisent séparément dans le journal de la pharmacie.
        ('facture',     'Facture encaissée'),
