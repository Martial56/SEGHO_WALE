...
def populate_sheet(ws, titre, columns, rows, periode_debut=None, periode_fin=None, genere_le=None):
    ...
    mid = max(n_cols // 2, 1)
    # « Généré le » ne tient sur la ligne que s'il reste au moins 2 colonnes de
    # chaque côté (étiquette + valeur) après « Mois : » — sinon on l'omet
    # plutôt que de tenter une fusion de cellules invalide (colonne de fin <
    # colonne de départ), qui plantait sur les feuilles à peu de colonnes.
    show_genere = n_cols >= 4
    ...
    if n_cols >= 2:
        mois_fin = mid if show_genere else n_cols
        if mois_fin > 2:
            ws.merge_cells(start_row=3, start_column=2, end_row=3, end_column=mois_fin)
