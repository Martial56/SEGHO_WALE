...
donnée (periode_debut, periode_fin : objets date) ; le moteur d'export
(voir export.py) se charge ensuite de produire le fichier Excel/CSV.
"""
...
def _soins_dispenses(periode_debut, periode_fin):
    ...
    qs = qs.filter(date_heure__date__lte=periode_fin).select_related(
        'patient', 'infirmier', 'departement'
    ).order_by('date_heure')
...
def _origine_hospitalisation(hosp):
    ...
    Même logique que l'ancien filtre gynéco du module Rapports : rattachement
    par département (module spécialisé 'gynecologie') ou, à défaut, par la
    spécialité du médecin de la consultation."""
    ...
    is_gyneco = False
    if departement and departement.modules_specialises.filter(code='gynecologie').exists():
        is_gyneco = True
...
def build_listing_activite_mo_xlsx(periode_debut, periode_fin, genere_le):
    ...
    (tableau âge/sexe + graphique), comme la fiche d'activité papier."""
    import io
...
REPORT_CATALOGUE = [
    ...
    },
    {
        ...
        'rapports': [
            {'slug': 'listing_activite_soins_mo', 'nom': 'Listing activité soins (MO)', 'icone': 'bi-hospital-fill',
