...
from employer.models import Employe as Medecin  # Employe unifié dans employer
...
def planning_modifier(request, pk):
    ...
    bureaux  = get_bureaux()
    medecins = Medecin.objects.filter(statut='actif', est_medecin=True).select_related('specialite').order_by('nom')
...
def planning_medecins_json(request):
    ...
    qs = Medecin.objects.filter(statut='actif', est_medecin=True).select_related('specialite')
    if q:
...
def _send_publication_email(planning, published_by):
    ...
    med_emails = set(Medecin.objects.filter(statut='actif', est_medecin=True).exclude(email='').values_list('email', flat=True))
