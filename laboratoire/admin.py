...
class LigneDemandeInlineFormSet(BaseInlineFormSet):
    ...
    une ligne d'examen. La vérification a lieu sur le formset car les lignes
    sont saisies en inline (hors du formulaire principal de la demande)."""

    def clean(self):
        super().clean()
        # self.instance = la DemandeExamen parente, avec le statut soumis
        statut = getattr(self.instance, "statut", None)
        if statut != "demande":
            return

        # Compter les lignes qui subsisteront après enregistrement :
        # formulaires valides, non vides, et non marqués pour suppression.
        lignes_restantes = 0
...
def action_envoyer_hprim(modeladmin, request, queryset):
    ...
    for demande in queryset:
        ...
        except Exception as exc:  # config absente, etc.
            erreurs += 1
